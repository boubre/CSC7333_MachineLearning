...
ClassNo C5Core::TreeClassify(DataRec Case, Tree DecisionTree)
{
  ClassNo  c;
...
void C5Core::FindLeaf(DataRec Case, Tree T, Tree PT, float Fraction)
{
  DiscrValue  v, Dv;
  ClassNo  c;
  float  NewFrac, BrWt[4];
  /*  Special case for winnowing cycles  */
  if ( T->NodeType && C5Skip(T->Tested) ) {
  ...
  case BrThresh:  /* test of continuous attribute */
    if ( Unknown(Case, T->Tested) ) {
      ...
      if ( NotApplic(Case, T->Tested) ) {
        ...
        BrWt[2] = (float)Interpolate(T, CVal(Case, T->Tested));
        BrWt[3] = 1 - BrWt[2];
        ForEach(v, 2, 3) {
...
/*                                                                 */
/*************************************************************************/
void C5Core::FollowAllBranches(DataRec Case, Tree T, float Fraction)
{
  DiscrValue  v;
  ForEach(v, 1, T->Forks) {
...
/*                                                                 */
/*************************************************************************/
ClassNo C5Core::RuleClassify(DataRec Case, CRuleSet RS)
{
  ClassNo  c, Best;
  float  TotWeight = 0;
  int    a, u = 1, d;
...
/*                                                                 */
/*************************************************************************/
int C5Core::FindOutcome(DataRec Case, Condition OneCond)
{
...
/*                   */
/*************************************************************************/
Boolean C5Core::Satisfies(DataRec Case, Condition OneCond)
{
...
/*                   */
/*************************************************************************/
Boolean C5Core::Matches(CRule R, DataRec Case)
{
  int d;
  ForEach(d, 1, R->Size) {
...
/*                   */
/*************************************************************************/
void C5Core::CheckActiveSpace(int N)
{
  if ( ActiveSpace <= N ) {
    ReallocateMem(Active, (ActiveSpace = N + 1), RuleNo);
  }
}
...
/*                   */
/*************************************************************************/
void C5Core::MarkActive(RuleTree RT, DataRec Case)
{
  DiscrValue  v;
  int    ri;
  RuleNo  r;
...
/*                   */
/*************************************************************************/
void C5Core::CheckUtilityBand(int* u, RuleNo r, ClassNo Actual, ClassNo Default)
{
  ClassNo  c;
...
/*                   */
/*************************************************************************/
ClassNo C5Core::BoostClassify(DataRec Case, int MaxTrial)
{
  ClassNo  c, Best;
  int    t;
  float  Total = 0;
...
/*                   */
/*************************************************************************/
ClassNo C5Core::SelectClass(ClassNo Default, Boolean UseCosts)
{
...
/*                      */
/*************************************************************************/
ClassNo C5Core::Classify(DataRec Case)
{
...
/*                      */
/*************************************************************************/
double C5Core::Interpolate(Tree T, ContValue Val)
{
...
/*                   */
/*************************************************************************/
void C5Core::FreeClassifier(int Trial)
{
  if ( Raw ) {
    FreeTree(Raw[Trial]);
    Raw[Trial] = Nil;
