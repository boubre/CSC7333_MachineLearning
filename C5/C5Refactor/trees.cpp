...
/*                   */
/*************************************************************************/
void C5Core::PrintTree(Tree T, String Title)
{
  int s;
  FindDepth(T);
  SubTree = 0;
...
/*                   */
/*************************************************************************/
void C5Core::Show(Tree T, int Sh)
{
  ...
  if ( T->NodeType ) {
    ...
    if ( Sh && Sh * TabSize + MaxLine(T) > Width ) {
      if ( ++SubTree >= SubSpace ) {
        SubSpace += 100;
        if ( SubDef ) {
          ReallocateMem(SubDef, SubSpace, Tree);
        } else {
          SubDef = AllocateMem(SubSpace, Tree);
        }
      }
      SubDef[SubTree] = T;
...
/*                   */
/*************************************************************************/
void C5Core::ShowBranch(int Sh, Tree T, DiscrValue v, DiscrValue BrNo)
{
  DiscrValue  Pv, Last;
  Attribute  Att;
  Boolean  FirstValue;
...
/*                   */
/*************************************************************************/
DiscrValue C5Core::Elements(Attribute Att, Set S, DiscrValue* Last)
{
...
/*                     */
/*************************************************************************/
void C5Core::Indent(int Sh, int BrNo)
{
  int  i;
  fprintf(Of, "\n");
  for ( i = 1 ; i <= Sh ; i++ ) {
...
/*                   */
/*************************************************************************/
Tree C5Core::Leaf(double* Freq, ClassNo NodeClass, CaseCount Cases, CaseCount Errors)
{
  Tree  Node;
  ClassNo  c;
...
/*                   */
/*************************************************************************/
void C5Core::Sprout(Tree T, DiscrValue Branches)
{
  T->Forks = Branches;
...
/*                   */
/*************************************************************************/
int C5Core::ExpandedLeafCount(Tree T)
{
  int    Sum = 0;
  DiscrValue  v, Dummy;
  if ( ! T->NodeType ) {
...
/*                   */
/*************************************************************************/
Tree C5Core::CopyTree(Tree T)
{
  DiscrValue  v;
  Tree  New;
  int    Bytes;
  New = AllocateMem(1, TreeRec);
  memcpy(New, T, sizeof(TreeRec));
  New->ClassDist = AllocateMem(MaxClass + 1, CaseCount);
  memcpy(New->ClassDist, T->ClassDist, (MaxClass + 1) * sizeof(CaseCount));
  if ( T->NodeType == BrSubset ) {
    Bytes = (MaxAttVal[T->Tested] >> 3) + 1;
    New->Subset = AllocateMem(T->Forks + 1, Set);
    ForEach(v, 1, T->Forks) {
      New->Subset[v] = AllocateMem(Bytes, unsigned char);
