...
void C5Core::PrintHeader(String Title)
{
  char  TitleLine[80];
  time_t  clock;
  int    Underline;
...
char C5Core::ProcessOption(int Argc, char* Argv[], char* Options)
{
  int    i;
  static int  OptNo = 1;
  if ( OptNo >= Argc )
...
/*                   */
/*************************************************************************/
void* C5Core::ProtMalloc(size_t Bytes)
{
  void* p = Nil;
...
void* C5Core::ProtRealloc(void* Present, size_t Bytes)
{
  void* p = Nil;
  if (!Bytes)
    return Nil;
  if (!Present)
    return ProtMalloc(Bytes);
...
void* C5Core::ProtCalloc(size_t Number, unsigned int Size)
{
  void* p = Nil;
...
void C5Core::FreeVector(void** V, int First, int Last)
{
  if ( V ) {
    while ( First <= Last ) {
...
DataRec C5Core::NewCase()
{
  ...
  if ( ! DataMem || DataMem->Allocated == DataBlockSize ) {
    ...
    DataMem->Head = AllocateMem(DataBlockSize * (MaxAtt + 2), AttValue);
    DataMem->Prev = Prev;
  }
...
void C5Core::FreeCases()
{
  DataBlock  Prev;
  while ( DataMem ) {
    Prev = DataMem->Prev;
    FreeMem(DataMem->Head);
    FreeMem(DataMem);
    DataMem = Prev;
  }
}
...
void C5Core::FreeLastCase(DataRec Case)
{
  DataMem->Allocated--;
}
...
void C5Core::ResetKR(int KRInit)
{
  KRFp = KRSp = 0;
  KRInit += 1000;
  while ( KRInit-- ) {
...
/*                   */
/*************************************************************************/
void C5Core::C5Error(int ErrNo, String S1, String S2)
{
  ...
  switch ( ErrNo ) {
  case -2:
    sprintf(Msg, "WARNING %s %s", S1, S2);
    break;
  case -1:
    sprintf(Msg, "FATAL ERROR %s %s", S1, S2);
    Quit = true;
    break;
  case NOFILE:
...
String C5Core::CaseLabel(CaseNo N)
{
  String      p;
...
/*                   */
/*************************************************************************/
FILE* C5Core::GetFile(String Extension, String RW)
{
  strcpy(Fn, FileStem);
  strcat(Fn, Extension);
  return fopen(Fn, RW);
...
/*                   */
/*************************************************************************/
int C5Core::Denominator(ContValue Val)
{
  ...
  for ( Mult = 100000 ; Mult >= 1 ; Mult /= 10 ) {
    RoundErr = fabs((Val * Mult) / Mult - Val);
...
/*                   */
/*************************************************************************/
int C5Core::GetInt(String S, int N)
{
  int  Result = 0;
  while ( N-- ) {
    if ( ! isdigit(*S) )
...
int C5Core::DateToDay(String DS)  /*  Day 1 is 0000/03/01  */
{
  int Year, Month, Day;
  if ( strlen(DS) != 10 )
    return 0;
...
void C5Core::DayToDate(int Day, String Date)
{
...
/*                   */
/*************************************************************************/
int C5Core::TimeToSecs(String TS)
{
  int Hour, Mins, Secs;
  if ( strlen(TS) != 8 )
    return -1;
...
void C5Core::SecsToTime(int Secs, String Time)
{
  int Hour, Mins;
  Hour = Secs / 3600;
  Mins = (Secs % 3600) / 60;
...
int C5Core::TStampToMins(String TS)
{
  int    Day, Sec, i;
...
/*                   */
/*************************************************************************/
void C5Core::CValToStr(ContValue CV, Attribute Att, String DS)
{
  int    Mins;
  if ( TStampVal(Att) ) {
    DayToDate(floor(CV / 1440) + TSBase, DS);
    DS[10] = ' ';
    Mins = (CV) - floor(CV / 1440) * 1440;
...
/*                   */
/*************************************************************************/
void C5Core::Check(float Val, float Low, float High)
{
  if ( Val < Low || Val > High ) {
...
/*                   */
/*************************************************************************/
int C5Core::UTF8CharWidth(unsigned char* U)
{
...
int C5Core::wcwidth(wchar_t ucs)
{
  ...
  while (max >= min) {
    ...
    if (combining[mid].last < ucs)
      ...
      if (combining[mid].first > ucs)
        max = mid - 1;
      else
        if (combining[mid].first <= ucs && combining[mid].last >= ucs)
          return 0;
  }
...
int C5Core::wcswidth(const wchar_t* pwcs, size_t n)
{
  int w, width = 0;
  for (; *pwcs && n-- > 0; pwcs++)
    if ((w = wcwidth(*pwcs)) < 0)
