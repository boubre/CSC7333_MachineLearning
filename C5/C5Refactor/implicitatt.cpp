...
/*                   */
/*************************************************************************/
void C5Core::ImplicitAtt(FILE* Nf)
{
#ifdef CUBIST
  _UNK.cval = UNKNOWN;
#else
  ...
  TStack = AllocateMem(TStackSize = 50, EltRec);
  TSN = 0;
  AttDef[MaxAtt] = AllocateMem(DefSize = 100, DefElt);
  DN = 0;
  ...
  if ( ! PreviousError ) {
    if ( DN == 1 && DefOp(AttDef[MaxAtt][0]) == OP_ATT &&
      ...
      C5Error(SAMEATT, AttName[ (long) DefSVal(AttDef[MaxAtt][0]) ], Nil);
    }
    if ( TStack[0].Type == 'B' ) {
...
/*                   */
/*************************************************************************/
void  C5Core::ReadDefinition(FILE* f)
{
  Boolean  LastWasPeriod = false;
  char  c;
  Buff = AllocateMem(BuffSize = 50, char);
  BN = 0;
  while ( true ) {
    c = InChar(f);
    if ( c == '|' )
...
/*                   */
/*************************************************************************/
void  C5Core::Append(char c)
{
  if ( c == ' ' && (! BN || Buff[BN - 1] == ' ' ) )
    return;
  if ( BN >= BuffSize ) {
    ReallocateMem(Buff, BuffSize += 50, char);
  }
  Buff[BN++] = c;
}
...
/*                   */
/*************************************************************************/
void C5Core::DefSyntaxError(String Msg)
{
  String  RestOfText;
  int    i = 10;
  if ( ! PreviousError ) {
    ...
    C5Error(BADDEF1, RestOfText, Msg);
    PreviousError = true;
  }
}

void C5Core::DefSemanticsError(int Fi, String Msg, int OpCode)
{
...
/*                   */
/*************************************************************************/
void C5Core::Dump(char OpCode, ContValue F, String S, int Fi)
{
  if ( Buff[Fi] == ' ' )
    Fi++;
  ...
  if ( DN >= DefSize - 1 ) {
    ReallocateMem(AttDef[MaxAtt], DefSize += 100, DefElt);
  }
...
void C5Core::DumpOp(char OpCode, int Fi)
{
  Dump(OpCode, 0, Nil, Fi);
}


Boolean C5Core::UpdateTStack(char OpCode, ContValue F, String S, int Fi)
{
  if ( TSN >= TStackSize ) {
    ReallocateMem(TStack, TStackSize += 50, EltRec);
  }
  switch ( OpCode ) {
  case OP_ATT:
...
AttValue  C5Core::EvaluateDef(Definition D, DataRec Case)
{
  ...
  for ( DN = 0 ; ; DN++) {
    ...
    case OP_DIV:
      ...
      if ( ! cv2 ||
        ...
        XStack[XSN - 2].cval = (cv1 * Mult) / Mult;
      }
      XSN--;
      break;
    case OP_MOD:
