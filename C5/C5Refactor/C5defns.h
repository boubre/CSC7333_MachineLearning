...
#ifndef DTC5_DEFINITIONS
#define DTC5_DEFINITIONS
...
#define C5AssertV(exp, msg, ret) \
  if (!(exp)) { \
  C5Error(-2, __FUNCTION__, msg); \
  return ret; \
  }
...
#define finite(x)    ((x) > -1.00E030 && (x) < 1.00E030)
#define AltRandom    rand()
#define  AltSeed(x)    srand48(x)

#define FreeMem(x)      {free(x); x=0;}
...
#define  AllocZero(N,T)    (T *) ProtCalloc(N, sizeof(T))
#define  AllocateMem(N,T)    AllocZero(N,T) /* for safety */
#define  ReallocateMem(V,N,T)    V = (T *) ProtRealloc(V, (N)*sizeof(T))
...
#define   C5Skip(a)    StatBit(a,EXCLUDE|SKIP)
...
#define  FreeUnlessNil(p)  if((p)!=Nil) FreeMem(p)
