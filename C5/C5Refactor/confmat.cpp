...
void C5Core::PrintConfusionMatrix(CaseNo* ConfusionMat)
{
  ...
  EntryWidth = (int)(floor(Log(EntryWidth + 100.0) / Log(10.0)) + 2);
...
void C5Core::PrintErrorBreakdown(CaseNo* ConfusionMat)
{
  ...
  ForEach(Row, 1, MaxClass) {
    ...
    ClassWidth = Max(ClassWidth, (int)strlen(ClassName[Row]));
  }
  EntryWidth = (int)(floor(Log(EntryWidth + 100.0) / Log(10.0)) + 2);
...
void C5Core::PrintUsageInfo(CaseNo* Usage)
{
  Attribute  Att, Best;
  float  Tests;
  Boolean  First = true;
  Tests = (float) Max(1, MaxCase + 1);

  std::set<int> printed;
  while ( true ) {
    Best = 0;
    ForEach(Att, 1, MaxAtt) {
      if ( Usage[Att] > Usage[Best]  && printed.find(Att) == printed.end())
        Best = Att;
    }
    if ( ! Best || Usage[Best] < 0.01 * Tests )
      break;
    if ( First ) {
      fprintf(Of, T_Usage);
      First = false;
