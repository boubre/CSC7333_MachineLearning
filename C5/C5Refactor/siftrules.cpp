...
/*                   */
/*************************************************************************/
void C5Core::SiftRules(float EstErrRate)
{
  RuleNo  r;
  int    d, *bp;
  CRule  R;
  ...
  Covered = AllocateMem(MaxCase + 1, Boolean);
  RuleIn  = AllocZero(NRules + 1, Boolean);
  /*  Set initial theory  */
  SetInitialTheory();
  Bits = AllocateMem(NRules + 1, float);
  ...
  if ( NRules >= MaxCase + 1 ) {
    ReallocateMem(List, NRules + 1, CaseNo);
  }
  ForEach(r, 1, NRules) {
    R = Rule[r];
    CodeLength = 0;
  ...
  DeltaErrs = AllocateMem(NRules + 1, float);
  TopClass = AllocateMem(MaxCase + 1, ClassNo);
  AltClass = AllocateMem(MaxCase + 1, ClassNo);
  TotVote  = AllocateMem(MaxCase + 1, int*);
...
/*                     */
/*************************************************************************/
void C5Core::InvertFires()
{
  RuleNo  r, Entry;
  int    j, Blocks, Extra;
  CaseNo  i;
  Byte*  p, *From, *To, *Next;
  CovByPtr = AllocateMem(MaxCase + 2, Byte*);
  Extra = NRules / 128;    /* max number of filler entries */
  CovByPtr[0] = 0;
  ForEach(i, 1, MaxCase + 1) {
    CovByPtr[i] = CovByPtr[i - 1] + CovBy[i - 1] + Extra;
  }
  CovByBlock = AllocateMem((size_t) CovByPtr[MaxCase + 1], Byte);
  ForEach(i, 0, MaxCase) {
  ...
  ReallocateMem(CovByBlock, To - CovByBlock, Byte);
  if ( CovByBlock != From ) {
...
/*                   */
/*************************************************************************/
float C5Core::CondBits(Condition C)
{
  Attribute  Att;
  float  Code = 0;
  int    Elts = 0;
...
void C5Core::CoverClass(ClassNo Target)
{
  CaseNo  i;
...
/*                   */
/*************************************************************************/
int C5Core::MessageLength(RuleNo NR, double RuleBits, float Errs)
{
  return (100.0 *
...
void C5Core::HillClimb()
{
  ...
  while ( true ) {
    ...
    if ( ! Toggle || DeleteOnly && RuleCount <= OriginalCount )
      break;
    Verbosity(1,
...
/*                   */
/*************************************************************************/
void C5Core::CountVotes(CaseNo i)
{
...
void C5Core::UpdateDeltaErrs(CaseNo i, double Delta, RuleNo Toggle)
{
...
/*                   */
/*************************************************************************/
CaseCount C5Core::CalculateDeltaErrs()
{
  RuleNo  r;
  CaseNo  i;
  double  Errs = 0;
...
void C5Core::PruneSubsets()
{
  ...
  Atts  = AllocateMem(MaxAtt + 1, Attribute);
  Bytes = AllocateMem(MaxAtt + 1, int);
...
/*                   */
/*************************************************************************/
void C5Core::SwapRule(RuleNo A, RuleNo B)
{
  CRule  Hold;
  Boolean  HoldIn;
  Hold    = Rule[A];
...
int C5Core::OrderByUtility()
{
  ...
  Drop = AllocateMem(NRules, RuleNo);
...
/*                   */
/*************************************************************************/
void C5Core::GenerateLogs(int MaxN)
{
  CaseNo  i;
  if ( LogCaseNo ) {
    ReallocateMem(LogCaseNo, MaxN + 2, double);
    ReallocateMem(LogFact, MaxN + 2, double);
  } else {
    LogCaseNo = AllocateMem(MaxN + 2, double);
    LogFact   = AllocateMem(MaxN + 2, double);
  }
  LogCaseNo[0] = -1E38;
  LogCaseNo[1] = 0;
