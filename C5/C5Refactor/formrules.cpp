...
/*                     */
/*************************************************************************/
CRuleSet C5Core::FormRules(Tree T)
{
  int    i;
  CRuleSet  RS;
  ...
  List   = AllocateMem(MaxCase + 2, CaseNo);
  Succ   = AllocateMem(MaxCase + 1, CaseNo);
  CBuffer   = AllocateMem(4 + (MaxCase + 1) + (MaxCase + 1) / 128, Byte);
  NRules = RuleSpace = 0;
...
/*                     */
/*************************************************************************/
void C5Core::SetupNCost()
{
  ClassNo  Real, Pred;
  double  AvErrCost = 0, ProbPred, ProbReal;
  NCost = AllocateMem(MaxClass + 1, float*);
  ForEach(Pred, 0, MaxClass) {
    NCost[Pred] = AllocateMem(MaxClass + 1, float);
...
void C5Core::PruneRule(Condition Cond[], ClassNo TargetClass)
{
...
/*                     */
/*************************************************************************/
void C5Core::AddToList(CaseNo* List, CaseNo N)
{
  Succ[N] = *List;
  *List   = N;
}
...
/*                     */
/*************************************************************************/
void C5Core::DeleteFromList(CaseNo* Before, CaseNo N)
{
  *Before = Succ[N];
}
...
/*                     */
/*************************************************************************/
int C5Core::SingleFail(CaseNo i)
{
  int    d;
  ForEach(d, 1, NCond) {
...
/*                     */
/*************************************************************************/
void C5Core::Increment(int d, CaseNo i, double* Total, double* Errors)
{
