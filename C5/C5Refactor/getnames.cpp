...
/*                   */
/*************************************************************************/
Boolean C5Core::ReadName(FILE* f, String s, int n, char ColonOpt)
{
  register char* Sp = s;
  register int  c;
  char    Msg[2];
  ...
  *LBp    = 0;
  ...
  do {
    ...
    if ( ++MaxClass >= ClassCeiling) {
      ClassCeiling += 100;
      ReallocateMem(ClassName, ClassCeiling, String);
    }
    ClassName[MaxClass] = strdup(Buffer);
  } while ( Delimiter == ',' );
  if ( Delimiter == ':' ) {
    /*  Thresholds for continuous class attribute  */
    ClassThresh = AllocateMem(ClassCeiling, ContValue);
    MaxClass = 0;
    do {
      ReadName(Nf, Buffer, 1000, ':');
      if ( ++MaxClass >= ClassCeiling) {
        ClassCeiling += 100;
        ReallocateMem(ClassThresh, ClassCeiling, ContValue);
      }
      ClassThresh[MaxClass] = strtod(Buffer, &EndBuff);
      if ( EndBuff == Buffer || *EndBuff != '\0' ) {
        C5Error(BADCLASSTHRESH, Buffer, Nil);
      } else
        if ( MaxClass > 1 &&
             ClassThresh[MaxClass] <= ClassThresh[MaxClass - 1] ) {
          C5Error(LEQCLASSTHRESH, Buffer, Nil);
        }
    } while ( Delimiter == ',' );
  }
  ...
  while ( ReadName(Nf, Buffer, 1000, ':') ) {
    ...
    if ( Which(Buffer, AttName, 1, MaxAtt) > 0 ) {
      C5Error(DUPATTNAME, Buffer, Nil);
    }
    if ( ++MaxAtt >= AttCeiling ) {
      AttCeiling += 100;
      ReallocateMem(AttName, AttCeiling, String);
      ReallocateMem(MaxAttVal, AttCeiling, DiscrValue);
      ReallocateMem(AttValName, AttCeiling, String*);
      ReallocateMem(SpecialStatus, AttCeiling, char);
      ReallocateMem(AttDef, AttCeiling, Definition);
      ReallocateMem(AttDefUses, AttCeiling, Attribute*);
    }
  ...
  if ( MaxClass == 1 || ClassThresh ) {
    ...
    if ( ClassAtt <= 0 || Exclude(ClassAtt) ) {
      C5Error(NOTARGET, ClassName[1], "");
    } else
      if ( ClassThresh &&
           ( ! Continuous(ClassAtt) ||
             StatBit(ClassAtt, DATEVAL | STIMEVAL | TSTMPVAL) ) ) {
        C5Error(BADCTARGET, ClassName[1], "");
      } else
        if ( ! ClassThresh &&
             ( Continuous(ClassAtt) || StatBit(ClassAtt, DISCRETE) ) ) {
          C5Error(BADDTARGET, ClassName[1], "");
        }
    FreeMem(ClassName[1]);
    if ( ! ClassThresh ) {
      FreeMem(ClassName);
      ...
      MaxClass++;
      ReallocateMem(ClassName, MaxClass + 1, String);
...
/*                   */
/*************************************************************************/
void C5Core::ExplicitAtt(FILE* Nf)
{
  char  Buffer[1000] = "", *p;
  DiscrValue  v;
  ...
  if ( ! ( ReadName(Nf, Buffer, 1000, ':') ) ) {
    C5Error(EOFINATT, AttName[MaxAtt], "");
  }
  MaxAttVal[MaxAtt] = 0;
  if ( Delimiter != ',' ) {
    /*  Typed attribute  */
        ...
        if ( ! strcmp(Buffer, "date") ) {
          ...
          if ( ! strcmp(Buffer, "time") ) {
            ...
            if ( ! memcmp(Buffer, "discrete", 8) ) {
              ...
              v = atoi(&Buffer[8]);
              if ( v < 2 ) {
                C5Error(BADDISCRETE, AttName[MaxAtt], "");
              }
              AttValName[MaxAtt] = AllocateMem(v + 3, String);
              ...
              if ( ! strcmp(Buffer, "ignore") ) {
                ...
                if ( ! strcmp(Buffer, "label") ) {
                  ...
                  C5Error(SINGLEATTVAL, AttName[MaxAtt], Buffer);
                }
  } else {
    ...
    do {
      ...
      if ( ++MaxAttVal[MaxAtt] >= ValCeiling ) {
        ValCeiling += 100;
        ReallocateMem(AttValName[MaxAtt], ValCeiling, String);
      }
...
/*                   */
/*************************************************************************/
int C5Core::Which(String Val, String* List, int First, int Last)
{
  int  n = First;
...
void C5Core::ListAttsUsed()
{
  ...
  if ( NUsed ) {
    AttDefUses[MaxAtt] = AllocateMem(NUsed + 1, Attribute);
      ...
      if ( AttDef[a] ) {
        for ( t = 0 ; DefOp(AttDef[a][t]) != OP_END ; t++ ) {
          if ( DefOp(AttDef[a][t]) == OP_STR ) {
            FreeMem(DefSVal(AttDef[a][t]));
          }
        }
        FreeMem(AttDef[a]);
        FreeMem(AttDefUses[a]);
      }
    }
    FreeMem(AttDef);
    AttDef = Nil;
