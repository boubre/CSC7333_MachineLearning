...
/*                   */
/*************************************************************************/
void C5Core::EstimateErrs(Tree T, CaseNo Fp, CaseNo Lp, int Sh, int Flags)
{
  ...
  LocalClassDist = AllocateMem(MaxClass + 1, double);
  ...
  BranchCases   = AllocateMem(T->Forks + 1, CaseCount);
  if ( Missing )
    UnitWeights = 0;
  TreeErrs = 0;
  Bp = Fp;
...
/*                   */
/*************************************************************************/
void C5Core::GlobalPrune(Tree T)
{
  int    DeltaLeaves, x;
  ...
  while ( MaxExtraErrs > 0 ) {
    ...
    Verbosity(2,
      ...
      if ( ! NXT || TotalExtraErrs > MaxExtraErrs )
        break;
...
/*                   */
/*************************************************************************/
void C5Core::InsertParents(Tree T, Tree P)
{
  DiscrValue  v;
  T->Parent = P;
  T->Errors = T->Leaves = 0;
...
/*                   */
/*************************************************************************/
void C5Core::CheckSubsets(Tree T, Boolean PruneDefaults)
{
  Set    HoldValues;
  ...
  if ( T->NodeType == BrSubset ) {
    ...
    HoldValues = AllocateMem(Bytes, Byte);
...
void C5Core::InitialiseExtraErrs()
{
  int i = 1;
...
/*                   */
/*************************************************************************/
float C5Core::ExtraErrs(CaseCount N, CaseCount E, ClassNo C)
{
  ClassNo  EC;
...
float C5Core::RawExtraErrs(CaseCount N, CaseCount E)
{
  float  Val0, Pr;
  if ( E < 1E-6 ) {
...
/*                   */
/*************************************************************************/
void C5Core::RestoreDistribs(Tree T)
{
  DiscrValue  v;
  ClassNo  c;
  if ( T->NodeType ) {
...
/*                   */
/*************************************************************************/
void C5Core::CompressBranches(Tree T)
{
  ...
  if ( T->NodeType ) {
    ...
    if ( S < T->Forks ) {
      /*  Compress!  */
      T->Subset   = AllocateMem(S + 1, Set);
      OldBranch   = T->Branch;
      T->Branch  = AllocateMem(S + 1, Tree);
...
void C5Core::SetGlobalUnitWeights(int LocalFlag)
{
