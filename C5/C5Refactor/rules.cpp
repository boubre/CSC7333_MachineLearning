...
/*                     */
/*************************************************************************/
Boolean C5Core::NewRule(Condition Cond[], int NCond, ClassNo TargetClass,
                ...
                CaseCount Cover, CaseCount Correct, float Prior)
{
  ...
  if ( ! Existing ) {
    ...
    Lhs = AllocateMem(Size + 1, Condition);
  ...
  if ( NRules >= RuleSpace ) {
    RuleSpace += 100;
    if ( RuleSpace > 100 ) {
      ReallocateMem(Rule,  RuleSpace, CRule);
      ReallocateMem(Fires, RuleSpace, Byte*);
      ForEach(r, RuleSpace - 100, RuleSpace - 1) {
        Fires[r] = Nil;
      }
    } else {
      Rule  = AllocateMem(RuleSpace, CRule);
      Fires = AllocZero(RuleSpace, Byte*);
    }
  }
  /*  Form the new rule  */
  Rule[NRules] = R = AllocateMem(1, RuleRec);
...
Byte* C5Core::Compress(int* L)
{
  ...
  Compressed = AllocateMem(p - CBuffer, Byte);
  memcpy(Compressed, CBuffer, p - CBuffer);
  return Compressed;
}

void C5Core::Uncompress(Byte* CL, int* UCL)
{
  int    i, Entry = 0;
  Byte*  p;
  memcpy(UCL, CL, 4);
...
/*                     */
/*************************************************************************/
void C5Core::ListSort(int* L, int Fp, int Lp)
{
...
/*                     */
/*************************************************************************/
Boolean C5Core::SameRule(RuleNo r, Condition Cond[], int NConds, ClassNo TargetClass)
{
  int  d, i, Bytes;
...
void C5Core::FreeRules(CRuleSet RS)
{
  int  ri;
...
/*                     */
/*************************************************************************/
void C5Core::PrintRules(CRuleSet RS, String Msg)
{
  int  r;
  fprintf(Of, "\n%s\n", Msg);
...
/*                     */
/*************************************************************************/
void C5Core::PrintCondition(Condition C)
{
