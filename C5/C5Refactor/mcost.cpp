  ...
  while ( ReadName(Cf, Name, 1000, ':') ) {
    ...
    if ( Pred > 0 && Real > 0 && Pred != Real && Val != 1 ) {
      ...
      if ( ! MCost ) {
        ...
        MCost = AllocateMem(MaxClass + 1, float*);
        ForEach(p, 1, MaxClass) {
          MCost[p] = AllocateMem(MaxClass + 1, float);
