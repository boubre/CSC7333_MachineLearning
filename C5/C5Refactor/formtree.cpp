...
/*                    */
/*************************************************************************/
void  C5Core::InitialiseTreeData()
{
  DiscrValue  v;
  Attribute  Att;
  DiscrValue  vMax;
  ...
  if ( SUBSET ) {
    ...
    Subset = AllocateMem(MaxAtt + 1, Set*);
    ForEach(Att, 1, MaxAtt) {
      if ( Discrete(Att) && Att != ClassAtt && ! C5Skip(Att) ) {
        Subset[Att] = AllocZero(MaxAttVal[Att] + 1, Set);
        ForEach(v, 0, MaxAttVal[Att]) {
          Subset[Att][v] = AllocateMem((MaxAttVal[Att] >> 3) + 1, Byte);
        }
      }
    }
    Subsets = AllocZero(MaxAtt + 1, int);
  }
  DList  = AllocateMem(MaxAtt, Attribute);
  NDList = 0;
  DFreq = AllocZero(MaxAtt + 1, double*);
  ForEach(Att, 1, MaxAtt) {
    if ( Att == ClassAtt || C5Skip(Att) || ! Discrete(Att) )
      continue;
    DList[NDList++] = Att;
    DFreq[Att] = AllocateMem(MaxClass * (MaxAttVal[Att] + 1), double);
  }
  ClassFreq = AllocZero(MaxClass + 1, double);
  ClassSum  = AllocateMem(MaxClass + 1, float);
  if ( BOOST ) {
    Vote      = AllocateMem(MaxClass + 1, float);
    TrialPred = AllocateMem(TRIALS, ClassNo);
  }
  if ( RULES ) {
    MostSpec     = AllocateMem(MaxClass + 1, CRule);
    PossibleCuts = AllocateMem(MaxAtt + 1, int);
  }
  /*  Check whether all attributes have many discrete values  */
  MultiVal = true;
  if ( ! SUBSET ) {
    for ( Att = 1 ; MultiVal && Att <= MaxAtt ; Att++ ) {
      if ( ! C5Skip(Att) && Att != ClassAtt ) {
  ...
  InitialiseExtraErrs();
  /*  Set up environment  */
  Waiting = AllocateMem(MaxAtt + 1, Attribute);
  vMax = Max(3, MaxDiscrVal + 1);
  GEnv.Freq = AllocateMem(vMax + 1, double*);
  ForEach(v, 0, vMax) {
    GEnv.Freq[v] = AllocateMem(MaxClass + 1, double);
  }
  GEnv.ValFreq = AllocateMem(vMax, double);
  GEnv.ClassFreq = AllocateMem(MaxClass + 1, double);
  GEnv.SRec = AllocateMem(MaxCase + 1, SortRec);
  if ( SUBSET ) {
    GEnv.SubsetInfo = AllocateMem(MaxDiscrVal + 1, double);
    GEnv.SubsetEntr = AllocateMem(MaxDiscrVal + 1, double);
    GEnv.MergeInfo = AllocateMem(MaxDiscrVal + 1, double*);
    GEnv.MergeEntr = AllocateMem(MaxDiscrVal + 1, double*);
    GEnv.WSubset   = AllocateMem(MaxDiscrVal + 1, Set);
    ForEach(v, 1, MaxDiscrVal) {
      GEnv.MergeInfo[v] = AllocateMem(MaxDiscrVal + 1, double);
      GEnv.MergeEntr[v] = AllocateMem(MaxDiscrVal + 1, double);
      GEnv.WSubset[v]   = AllocateMem((MaxDiscrVal >> 3) + 1, Byte);
    }
  }
}

void C5Core::FreeTreeData()
{
  Attribute  Att;
  DiscrValue  vMax;
  FreeUnlessNil(Raw);
  ...
  FreeMem(GEnv.ValFreq);
  FreeMem(GEnv.ClassFreq);
  FreeUnlessNil(GEnv.SRec);
  if ( GEnv.SubsetInfo ) {
    FreeMem(GEnv.SubsetInfo);
    FreeMem(GEnv.SubsetEntr);
...
/*                    */
/*************************************************************************/
void C5Core::FormTree(CaseNo Fp, CaseNo Lp, int Level, Tree* Result)
{
  ...
  MaxLeaves = ( LEAFRATIO > 0 ? (LEAFRATIO * Cases) : 1E6 );
  *Result = Node =
  ...
  if ( BestAtt == None ) {
    ...
    Verbosity(1, fprintf(Of, "\tbest attribute %s", AttName[BestAtt]);
...
/*                    */
/*************************************************************************/
void C5Core::SampleEstimate(CaseNo Fp, CaseNo Lp, CaseCount Cases)
{
...
/*                    */
/*************************************************************************/
void C5Core::Sample(CaseNo Fp, CaseNo Lp, CaseNo N)
{
  CaseNo  i, j;
  double  Interval;
...
/*                    */
/*************************************************************************/
Attribute C5Core::ChooseSplit(CaseNo Fp, CaseNo Lp, CaseCount Cases, Boolean Sampled)
{
  Attribute  Att;
  int i, j;
...
void C5Core::ProcessQueue(CaseNo WFp, CaseNo WLp, CaseCount WCases)
{
  Attribute  Att;
  float  GR;
  for ( ; NWaiting > 0 ; ) {
...
/*                    */
/*************************************************************************/
Attribute C5Core::FindBestAtt(CaseCount Cases)
{
...
/*                    */
/*************************************************************************/
void C5Core::EvalDiscrSplit(Attribute Att, CaseCount Cases)
{
  DiscrValue  v, NBr;
  Gain[Att] = None;
  if ( C5Skip(Att) || Att == ClassAtt )
...
/*                    */
/*************************************************************************/
void C5Core::Divide(Tree T, CaseNo Fp, CaseNo Lp, int Level)
{
...
/*                    */
/*************************************************************************/
CaseNo C5Core::Group(DiscrValue V, CaseNo Bp, CaseNo Ep, Tree TestNode)
{
  CaseNo  i;
  Attribute  Att;
  ContValue  Thresh;
...
/*                    */
/*************************************************************************/
CaseCount C5Core::SumWeights(CaseNo Fp, CaseNo Lp)
{
  double  Sum = 0.0;
  CaseNo  i;
...
/*                    */
/*************************************************************************/
CaseCount C5Core::SumNocostWeights(CaseNo Fp, CaseNo Lp)
{
  double  Sum = 0.0;
  CaseNo  i;
  assert(Fp >= 0 && Lp >= Fp - 1 && Lp <= MaxCase);
...
/*                   */
/*************************************************************************/
void C5Core::FindClassFreq(double* CF, CaseNo Fp, CaseNo Lp)
{
  ClassNo  c;
  CaseNo  i;
  assert(Fp >= 0 && Lp >= Fp && Lp <= MaxCase);
...
/*                   */
/*************************************************************************/
void C5Core::FindAllFreq(CaseNo Fp, CaseNo Lp)
{
  ClassNo  c;
  CaseNo  i;
  Attribute  Att, a;
