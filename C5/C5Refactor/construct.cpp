...
/*  to reweigh the training cases, and the reweighing is reversed   */
...
/*                   */
/*************************************************************************/
void C5Core::ConstructClassifiers()
{
  ...
  ForEach(Trial, 0, TRIALS - 1 ) {
    ...
    /*  Prune the raw tree to minimize expected misclassification cost  */
    ...
    if ( ErrWt < 0.1 ) {
      ...
      if ( Trial && NoStructure || ErrWt / Cases >= 0.49 ) {
        ...
        b = ExtraErrWt / Errs;
        /*  Normalize so that sum of weights equals number of cases  */
...
/*************************************************************************/
/*                    */
/*  Initialize the weight of each case         */
/*                    */
/*************************************************************************/
void C5Core::InitialiseWeights()
{
  CaseNo  i;
  if ( CostWeights ) {
...
void C5Core::Evaluate(int Flags)
{
  if ( TRIALS == 1 ) {
...
void C5Core::EvaluateSingle(int Flags)
{
  ClassNo  RealClass, PredClass;
  int x, u, SaveUtility;
  CaseNo i, RawErrs = 0, Errs = 0;
  ...
  if ( CMInfo ) {
    ...
    _ConfusionMat = AllocZero((MaxClass + 1) * (MaxClass + 1), CaseNo);
  }
  if ( UsageInfo ) {
    FreeMem(_Usage);
    _Usage = AllocZero(MaxAtt + 1, CaseNo);
  }
  ...
  if ( UTILITY && RULES ) {
    ...
    UtilBand = AllocateMem(UTILITY, int);
    if ( MCost ) {
      UtilCost = AllocZero(UTILITY, double);
    }
    ForEach(u, 1, UTILITY - 1) {
      UtilBand[u] = (RuleSet[0]->SNRules * u / (float) UTILITY);
    }
  }
  fprintf(Of, "\n");
  ForEach(x, 0, 2) {
  ...
  ForEach(i, 0, MaxCase) {
    ...
    if ( CMInfo ) {
      _ConfusionMat[RealClass * (MaxClass + 1) + PredClass]++;
    }
    if ( UsageInfo ) {
      RecordAttUsage(Case[i], _Usage);
    }
  }
  putc('\t', Of);
  if ( RULES ) {
    ...
    Verbosity(1, { fprintf(Of, "  %4d %4d(%4.1f%%)  ", TreeSize(Raw[0]), RawErrs, 100 * RawErrs / Tests); });
  ...
  fprintf(Of, "   <<\n");
  if ( CMInfo ) {
    PrintConfusionMatrix(_ConfusionMat);
    //FreeMem(ConfusionMat);
  }
  if ( UsageInfo ) {
    PrintUsageInfo(_Usage);
    //FreeMem(Usage);
  }
  if ( UtilErr ) {
    if ( ! XVAL ) {
...
void C5Core::EvaluateBoost(int Flags)
{
  ClassNo  RealClass, PredClass;
  int    t;
...
/*                    */
/*************************************************************************/
void C5Core::RecordAttUsage(DataRec Case, int* Usage)
{
  Attribute  Att;
  int    i;
