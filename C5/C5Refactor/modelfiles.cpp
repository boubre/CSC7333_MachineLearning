...
/*                   */
/*************************************************************************/
void C5Core::CheckFile(String Extension, Boolean Write)
{
  static char*  LastExt = "";
...
/*                   */
/*************************************************************************/
void C5Core::WriteFilePrefix(String Extension)
{
  time_t  clock;
  struct tm*  now;
  if ( ! (TRf = GetFile(Extension, "w")) ) {
    C5Error(NOFILE, Fn, E_ForWrite);
  }
  clock = time(0);
  now = localtime(&clock);
  now->tm_mon++;
...
/*                   */
/*************************************************************************/
void C5Core::ReadFilePrefix(String Extension)
{
...
/*                   */
/*************************************************************************/
void C5Core::SaveTree(Tree T, String Extension)
{
...
/*                     */
/*************************************************************************/
void C5Core::SaveRules(CRuleSet RS, String Extension)
{
  int    ri, d;
  CRule  R;
  Condition  C;
...
/*                   */
/*************************************************************************/
void C5Core::AsciiOut(String Pre, String S)
{
  fprintf(TRf, "%s\"", Pre);
  while ( *S ) {
    if ( *S == '"' || *S == '\\' )
...
void C5Core::ReadHeader()
{
  ...
  while ( true ) {
    ...
    case ATTP:
      ...
      if ( ! Att || Exclude(Att) ) {
        C5Error(MODELFILE, E_MFATT, Unquoted);
      }
      break;
    case ELTSP:
      MaxAttVal[Att] = 1;
...
/*                   */
/*************************************************************************/
Tree C5Core::GetTree(String Extension)
{
...
Tree C5Core::InTree()
{
  ...
  do {
    ...
    case CLASSP:
      ...
      if ( ! T->Leaf )
        C5Error(MODELFILE, E_MFCLASS, Unquoted);
      break;
    case ATTP:
      Unquoted = RemoveQuotes(PropVal);
    ...
    case FORKSP:
      ...
      break;
    case FREQP:
      T->ClassDist = AllocateMem(MaxClass + 1, CaseCount);
      p = PropVal + 1;
  ...
  if ( T->ClassDist ) {
    ...
    T->ClassDist = AllocateMem(1, CaseCount);
  }
  if ( T->NodeType ) {
...
/*                   */
/*************************************************************************/
CRuleSet C5Core::GetRules(String Extension)
{
  CheckFile(Extension, false);
  return InRules();
}
...
CRuleSet C5Core::InRules()
{
  CRuleSet  RS;
  RuleNo  r;
  char  Delim;
  RS = AllocateMem(1, RuleSetRec);
  do {
    switch ( ReadProp(&Delim) ) {
    case ERRORP:
      return Nil;
  ...
  /*  Read each rule  */
  RS->SRule = AllocateMem(RS->SNRules + 1, CRule);
...
CRule C5Core::InRule()
{
  ...
  R->Lhs = AllocateMem(R->Size + 1, Condition);
  ForEach(d, 1, R->Size) {
    R->Lhs[d] = InCondition();
  }
...
Condition C5Core::InCondition()
{
  Condition  C;
  char  Delim;
  int    X;
  double  XD;
  C = AllocateMem(1, CondRec);
  do {
    switch ( ReadProp(&Delim) ) {
    case ERRORP:
      return Nil;
    ...
    case VALP:
      if ( Continuous(C->Tested) ) {
        ...
        if ( ! C->TestValue )
          C5Error(MODELFILE, E_MFATTVAL, Unquoted);
      }
      break;
    case ELTSP:
...
/*                   */
/*************************************************************************/
int C5Core::ReadProp(char* Delim)
{
  int    c, i;
  char*  p;
  Boolean  Quote = false;
  ...
  *p = '\00';
  for ( p = PropVal ; ((c = fgetc(TRf)) != ' ' && c != '\n') || Quote ; ) {
    ...
    if ( (i = p - PropVal) >= PropValSize ) {
      ReallocateMem(PropVal, (PropValSize += 10000) + 3, char);
      p = PropVal + i;
    }
    *p++ = c;
    if ( c == '\\' ) {
...
String C5Core::RemoveQuotes(String S)
{
  char*  p, *Start;
  p = Start = S;
  for ( S++ ; *S != '"' ; S++ ) {
...
Set C5Core::MakeSubset(Attribute Att)
{
  int    Bytes, b;
  char*  p;
  Set    S;
  ...
  for ( p = PropVal ; *p ; ) {
    ...
    if ( ! b )
      C5Error(MODELFILE, E_MFATTVAL, p);
    SetBit(b, S);
    for ( p += strlen(p) ; *p != '"' ; p++ );
    p++;
    if ( *p == ',' )
...
/*                     */
/*************************************************************************/
void C5Core::StreamIn(String S, int n)
{
  while ( n-- )
    *S++ = getc(TRf);
}
