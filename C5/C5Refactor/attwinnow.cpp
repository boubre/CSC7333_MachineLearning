...
void C5Core::WinnowAtts()
{
  ...
  ForEach(Att, 1, MaxAtt) {
    if ( Att != ClassAtt && ! C5Skip(Att) && ! Split[Att] ) {
      SpecialStatus[Att] ^= SKIP;
      Removed++;
    }
  }
...
/*                   */
/*************************************************************************/
float C5Core::TrialTreeCost(Boolean FirstTime)
{
  Attribute  Att;
...
/*                   */
/*************************************************************************/
float C5Core::ErrCost(Tree T, CaseNo Fp, CaseNo Lp)
{
  CaseNo  i;
  float  ErrCost = 0;
  ClassNo  Pred;
...
/*                   */
/*************************************************************************/
void C5Core::ScanTree(Tree T, Boolean* Used)
{
  DiscrValue  v;
  if ( T->NodeType ) {
    Used[T->Tested] = true;
