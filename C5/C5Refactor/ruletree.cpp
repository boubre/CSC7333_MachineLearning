...
/*                                                                 */
/*************************************************************************/
void C5Core::ConstructRuleTree(CRuleSet RS)
{
  int    r, c;
  RuleNo*  All;
  Test = AllocateMem((TestSpace = 1000), Condition);
  NTest = 0;
  All = AllocateMem(RS->SNRules, RuleNo);
  ...
  TestOccur = AllocateMem(NTest, int);
...
/*                                                                 */
/*************************************************************************/
void C5Core::SetTestIndex(Condition C)
{
  int    t;
  Condition  CC;
  Attribute  Att;
  ...
  if ( NTest >= TestSpace ) {
    ReallocateMem(Test, (TestSpace += 1000), Condition);
  }
  Test[NTest] = C;
  C->TestI = NTest++;
}
...
/*                                                                 */
/*************************************************************************/
RuleTree C5Core::GrowRT(RuleNo* RR, int RRN, CRule* Rule)
{
  RuleTree  Node;
  RuleNo  r, *LR;
  ...
  if ( FP ) {
    Node->Fire = AllocateMem(FP + 1, RuleNo);
  ...
  Node->Branch = AllocateMem(Node->Forks + 1, RuleTree);
  LR = AllocateMem(RRN, RuleNo);
...
/*                                                                 */
/*************************************************************************/
int C5Core::DesiredOutcome(CRule R, int TI)
{
  int    c;
  Boolean  ContinTest;
...
/*                                                                 */
/*************************************************************************/
int C5Core::SelectTest(RuleNo* RR, int RRN, CRule* Rule)
{
  int    c, cc, ri;
  RuleNo  r;
...
/*                                                                 */
/*************************************************************************/
void C5Core::FreeRuleTree(RuleTree RT)
{
  int    b;
  if ( ! RT )
    return;
