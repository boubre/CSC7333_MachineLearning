...
void C5Core::CrossVal()
{
  ...
  Blocked   = AllocateMem(MaxCase + 1, DataRec);
...
void C5Core::Prepare()
{
  ...
  ClassNo  Group;
  Temp = AllocateMem(MaxCase + 1, CaseNo);
  ForEach(i, 0, MaxCase) {
    Temp[i] = i;
  }
  Shuffle(Temp);
...
float C5Core::SE(float sum, float sumsq, int no)
{
  float mean;
  mean = sum / no;
