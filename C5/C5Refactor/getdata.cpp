...
//
//  This file is part of C5Core.0 GPL Edition, a single-threaded version
//  of C5Core.0 release 2.07.
//
//  C5Core.0 GPL Edition is free software: you can redistribute it and/or
//  modify it under the terms of the GNU General Public License as
//  published by the Free Software Foundation, either version 3 of the
//  License, or (at your option) any later version.
//
//  C5Core.0 GPL Edition is distributed in the hope that it will be useful,
//  but WITHOUT ANY WARRANTY; without even the implied warranty of
//  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU
//  General Public License for more details.
//
//  You should have received a copy of the GNU General Public License
//  (gpl.txt) along with C5Core.0 GPL Edition.  If not, see
...
#define XError(a,b,c)  if (! SuppressErrorMessages) C5Error(a,b,c)
...
/*                   */
/*************************************************************************/
void C5Core::GetData(FILE* Df, Boolean Train, Boolean AllowUnknownClass)
{
  DataRec  DVec;
  ...
  if ( Train || ! Case ) {
    ...
    Case = AllocateMem(1, DataRec);  /* for error reporting */
  } else {
    CaseSpace = MaxCase + 1;
    MaxCase++;
  }
  ...
  while ( (DVec = GetDataRec(Df, Train)) ) {
    ...
    if ( MaxCase >= CaseSpace ) {
      CaseSpace += Inc;
      ReallocateMem(Case, CaseSpace + 1, DataRec);
    }
...
/*                   */
/*************************************************************************/
DataRec C5Core::GetDataRec(FILE* Df, Boolean Train)
{
  Attribute  Att;
...
/*                                                                       */
/*************************************************************************/
CaseNo C5Core::CountData(FILE* Df)
{
  char        Last = ',';
  int         Count = 0, Next;
  while ( true ) {
...
/*                   */
/*************************************************************************/
int C5Core::StoreIVal(String S)
{
  int    StartIx, Length;
  if ( (Length = strlen(S) + 1) + IValsOffset > IValsSize ) {
    if ( IgnoredVals ) {
      ReallocateMem(IgnoredVals, IValsSize += 32768, char);
    } else {
      IValsSize   = 32768;
      IValsOffset = 0;
      IgnoredVals = AllocateMem(IValsSize, char);
    }
  }
  StartIx = IValsOffset;
...
/*                   */
/*************************************************************************/
void C5Core::CheckValue(DataRec DVec, Attribute Att)
{
  ContValue  Cv;
  Cv = CVal(DVec, Att);
  if ( ! finite(Cv) ) {
    C5Error(BADNUMBER, AttName[Att], "");
